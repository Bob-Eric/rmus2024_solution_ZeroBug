...
def classify(roi):
    ...
    cv2.waitKey(0)
    for img in to_match:
        scores.append([np.sum(img == tpl) / (sz[0] * sz[1]) for tpl in my_templates])
    scores = np.array(scores)
    maxloc = np.unravel_index(np.argmax(scores), scores.shape)
    print(maxloc[1])
    print(f"max likelihood in match templates: {scores[maxloc]:.2g}")
    cv2.putText(roi, str(maxloc[1] + 1), (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    return

def get_digits(img_binary, img_depth, camera_matrix):
    cv2.imshow("img_binary", img_binary)

    """ floodFill to remove the noise """
    _, img_fill, _, _ = cv2.floodFill(img_binary, None, (0, 0), 255)
    img_fill = cv2.bitwise_not(img_fill)
    ## close operation to fill thin black gaps
    img_close = cv2.morphologyEx(img_fill, cv2.MORPH_CLOSE, (3, 3))
    """ get contours """
    contours, _ = cv2.findContours(
        img_close, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
    )
    """ filter out small contours """
    for i in reversed(range(len(contours))):
        x, y, w, h = cv2.boundingRect(contours[i])
        if w * h < 100:     # can be 80 or even less but may cause false positive
            contours.pop(i)
        else:
            print(f"\ncontour {i} area: {w * h}")
            classify(img_close[y:y+h, x:x+w])
    
    
    # M = np.array(camera_matrix)
    # np.linalg.inv(M) * depth_img

    frame = cv2.cvtColor(img_fill, cv2.COLOR_GRAY2BGR)
    ...
    if len(contours) > 0:
        ...
        for contour in contours:
            ...
            if len(approx) != 4:
                continue
            """ find warped rect """
            testImg = cv2.drawContours(testImg, [approx], -1, (0, 255, 0), 2)
            # quads.append(approx)
